...
OffsetUV_X_1("OffsetUV_X_1", Range(-1, 1)) = 0.774
OffsetUV_Y_1("OffsetUV_Y_1", Range(-1, 1)) = -0.018
OffsetUV_ZoomX_1("OffsetUV_ZoomX_1", Range(0.1, 10)) = 0.823
...
float2 _Simple_Displacement_1 = SimpleDisplacementUV(i.texcoord,_DisplacementPack_2.r*_DisplacementPack_2.a,0,-0.3);
float2 DistortionUV_1 = DistortionUV(_Simple_Displacement_1,108.043,107.129,0.911,0.587,0.354);
float2 DistortionUV_2 = DistortionUV(DistortionUV_1,10,20.103,0.139,0.13,1);
float2 ZoomUV_1 = ZoomUV(DistortionUV_2,3.486,0.5,0.5);
float2 FishEyeUV_1 = FishEyeUV(ZoomUV_1,0.254);
float4 _DisplacementPack_1 = DisplacementPack(FishEyeUV_1,DisplacementPack_1,0.61,-0.773,-1.547,1,1);
float4 _ColorFilters_1 = ColorFilters(_DisplacementPack_1,float4(1.3,0.08,0.07,0.1),float4(0.19,0.89,0.03,0.19),float4(-0.01,0.11,0.57,0.47),1);
...
float4 MaskRGBA_1=_ColorFilters_1;
MaskRGBA_1.a = lerp(float4(1,1,0,1).r, 1 - float4(1,1,0,1).r ,0);
float4 NewTex_2 = tex2D(_NewTex_2, i.texcoord);
float4 OperationBlend_2 = OperationBlend(MaskRGBA_1, NewTex_2, 1); 
