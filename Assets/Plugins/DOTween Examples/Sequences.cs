...
public class Sequences : MonoBehaviour
{
	...
	IEnumerator Start()
	{
        ...
        s.Append(cube.DOMoveX(6, duration))
        .Append(cube.DOMoveX(-6, duration))
        // Insert a rotation tween which will last half the duration
        // and will loop forward and backward twice
        //.Insert(0, cube.DORotate(new Vector3(0, 45, 0), duration / 2).SetEase(Ease.InQuad).SetLoops(2, LoopType.Yoyo))
        // Add a color tween that will start at half the duration and last until the end
        .Insert(duration / 2, cube.GetComponent<Renderer>().material.DOColor(Color.yellow, duration / 2));
